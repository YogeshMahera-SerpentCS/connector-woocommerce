...
class WooBaseExporter(AbstractComponent):
    """ Base exporter for Woo """

    _name = 'woo.base.exporter'
    _inherit = ['base.exporter', 'base.woo.connector']
    _usage = 'record.exporter'

    def __init__(self, working_context):
        super(WooBaseExporter, self).__init__(working_context)
        self.binding = None
    ...
    def _should_import(self):
        ...
        in Woo and the last sync date in Odoo,
        if the former is more recent, schedule an import
        to not miss changes done in Woo.
        """
        assert self.binding
        ...
        if not sync:
            return True
        result = self.backend_adapter.is_woo_record(self.external_id)
        return result
    ...
    def _after_export(self):
        """ Can do several actions after exporting a record on woo """
        pass


class WooExporter(AbstractComponent):
    """ A common flow for the exports to Woo """

    _name = 'woo.exporter'
    _inherit = 'woo.base.exporter'

    def __init__(self, working_context):
        super(WooExporter, self).__init__(working_context)
        self.binding = None
    ...
    def _retry_unique_violation(self):
        ...
        record created by :meth:`_export_dependency`), resulting in:
            ...
            constraint "woo_product_product_odoo_uniq"
    ...
    def _export_dependency(self, relation, binding_model,
                           ...
                           binding_field='woo_bind_ids',
                           binding_extra_vals=None):
        """
        Export a dependency. The exporter class is a subclass of
        ``WooExporter``. If a more precise class need to be defined,
        ...
        if wrap and hasattr(relation, binding_field):
            ...
            # woo.product.product and we are exporting it, but we need to
            ...
            # If woo_bind_ids does not exist we are typically in a
    ...
    def _update_data(self, map_record, fields=None, **kwargs):
        ...
        res = map_record.values(fields=fields, **kwargs)
        return res
    ...
    def _run(self, fields=None):
        ...
        if self.external_id:
            ...
            if not record:
                return _('Nothing to export.')
            bind = self._create(record)
            # Need of more test cases.
            if bind.get('id'):
                self.external_id = bind.get('id')
            else:
                self.external_id = bind.get(list(bind.keys())[0]).get('id')
            assert self.external_id, "External Id is not Found"
        return _('Record exported with ID %s on Woo.') % self.external_id
